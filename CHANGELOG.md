...
- `taskPilot.globalMenu` is now an `application`-scoped setting: it can only be set in User settings, so a workspace `settings.json` can no longer shadow the value the export/promote commands write to. (#11597)
...
### Fixed
- The Marketplace publish job now requires a `push` event in addition to a `v*` tag ref, so manually dispatching the workflow on a tag ref runs the test gate only and can no longer publish. (#11548)
